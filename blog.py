    ...
    def log_user_in(self):
        ...
        for user in self.users:
            ...
            if user.username == username and user.check_password(password):
                # If credentials are correct, set blog's user to that user instance
                self.current_user = user
                print(f"{user} has logged in")
                break
        # If no users in the blog have that username/password,  flash invalid credentials.
        else:
 ...
 # Method to add a new post to the blog authored by the logged in user
    def create_new_post(self):
        ...
        if self.current_user is not None:
            # Get the title and body of the new post from the en 
...
def run_blog():
    ...
    # If there is a logged in user (aka current_user is not None, it is a user instance)
            ...
            print('1. Log Out\n2. Create A New Post\n3. View All Posts\n4. View Single Post\n5. Edit A Post\n6. Delete A Post')
            to_do = input('Which option would you like to do? ')
            while to_do not in {'1', '2', '3', '4', '5', '6', 'or 7'}:
